...
import { reportStatus } from '../../helper/context/agent-context/agentreport.service';
...
const AgentDashboardTable = () => {
	...
	const deleteReport = (reportIDParam) => {
		const deletedReport = specificAgencyReport.filter(
			(report) => report.id !== reportIDParam
		);
	...
	return (
		<>
					...
					{specificAgencyReport.map((details, index) => {
						return (
							<tr key={details.id}>
								...
								<td>
									<select
										className={style.agentTableReportSelect}
										onChange={(e) =>
											handleChangeReportStatus(details.id, e.target.value)
										}
									>
			...
			<AgentDashBoardModal
				...
				show={show}
				viewspecificAgencyReport={viewspecificAgencyReport}
			/>
		</>
	);
};
